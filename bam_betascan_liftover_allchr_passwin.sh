...
echo "There were $unfilt_tot SNPs analyzed" > $liftout/$species'_filtering.txt'

### output filtered variant counts ####
n_HWE_filt=$(cat $mafpath/*'_badHWE.txt' | wc -l)
echo "$n_HWE_filt SNPs didn't pass HWE pvalue filter" >> $liftout/$species'_filtering.txt'

n_SB_filt=$(cat $mafpath/*'_badSB.txt' | wc -l)
echo "$n_SB_filt SNPs didn't pass SB filter" >> $liftout/$species'_filtering.txt'
...
if [ $filt_both_homo == T ]; then
  n_homoz_filt=$(cat $maffile*'.geno_lacking_homoz.txt' | wc -l)
  echo "$n_homoz_filt SNPs removed that didn't have both homozygous classes" >> $liftout/$species'_filtering.txt'
fi

echo "Likely overlap between filters, $n_SNP_filt total SNPs were removed" >> $liftout/$species'_filtering.txt'

pct_filt=$(bc <<< "scale=2; $n_SNP_filt/$unfilt_tot")


echo "The overall fraction of filtered SNPs was: $pct_filt" >> $liftout/$species'_filtering.txt'

## output all the SNPs without both homozygous classes (most snps) 
## we will use this to check best snp in window later
cat $maffile*'.geno_both_homoz.txt' > $liftout/$species'.geno_both_homoz.txt'
...
if [ $species == 'YRI' ] 
  ...
  #concatinate the chrs from pre-liftover files
    ...
    #rm $wd/*toliftover.bed 
      ...
      n_unlifted=$(cat $liftpath/$species'_to_hg38_'*'_win'$win'_smoothed_Unmapped.bed' | wc -l)

      n_lifttot=$(($n_unlifted+$n_lifted))

      fract_nolift=$(bc <<< "scale=2; $n_unlifted/$n_lifttot")

      echo "The overall fraction of windows that failed liftover was: $fract_nolift" >> $liftout/$species'_filtering.txt'
